...
namespace REP_SMS_CONTROLLER_F
{
    public class CL_SMS
    {
    ...
    public class CL_MSG
    {
        ...
        DataTable tab_message = new DataTable();
        public CL_MSG()
        {
            CreateDB();
        ...
        public string GetTime(string messages)
        {
            ...
            if(!(from <= 0 && to <= 0)) 
            { 
                time = messages.Substring(from, to - from).Replace(", 'Date': '", "").Trim(); 
            }
        ...
        public string GetContent(string message)
        {
            ...
            if (!(from <= 0 && to <= 0))
            { content = message.Substring(from, to - from).Replace(", 'Content': '", "").Trim(); }
            return content;
        }
